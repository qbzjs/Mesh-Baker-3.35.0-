...
namespace xasset
{
    ...
    public sealed class DownloadFiles : Operation
    {
        ...
        public DownloadFiles(bool isHighPriority)
        {
        ...
        public override void Start()
        {
            ...
            _progressing.Clear();
            _downloaded.Clear();
            foreach (var info in files) totalSize += info.size;

            if (files.Count > 0)
                foreach (var item in files)
                {
                    var download = Download.DownloadAsync(item, null, highPriority);
        ...
        public void Retry()
        {
            ...
            {
                Download.Retry(download, highPriority);
                _progressing.Add(download);
            }
            ...
            if (_progressing.Count > 0)
            {
                ...
                for (var index = 0; index < _progressing.Count; index++)
                {
                    ...
                    if (item.isDone)
                    {
                        ...
                        if (item.status == DownloadStatus.Failed) _errors.Add(item);
                    }
                    else
                    {
                ...
                foreach (var item in _downloaded) len += item.downloadedBytes;
