...
namespace xasset
{
    internal class DownloadBundle : Bundle
    {
        ...
        {
            _download = Download.DownloadAsync(Versions.GetDownloadInfo(info.nameWithAppendHash, info.hash, info.size), OnDownloaded);
        }

        private void OnDownloaded(Download obj)
        {
            if (obj.status != DownloadStatus.Success)
            {
                Finish(obj.error);
                return;
            }
