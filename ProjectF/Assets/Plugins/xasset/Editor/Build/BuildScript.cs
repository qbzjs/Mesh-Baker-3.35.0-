...
namespace xasset.editor
{
    public static class BuildScript
    {
        ...
        public static void BuildBundles(int versionCode, string path)
        {
            BuildBundles(new BuildTask(versionCode, path));
        }
        ...
        {
            if (string.IsNullOrEmpty(path)) path = $"Players/{Settings.GetPlatformName()}";
        ...
        public static void CopyToStreamingAssets(string versionName = null, string buildPath = null)
        {
            ...
            var versions = BuildVersions.Load(Settings.GetBuildPath(Versions.Filename, buildPath));
            var bundles = settings.GetBundlesInBuild(versions, versionName, buildPath);
            foreach (var bundle in bundles) Copy(bundle.nameWithAppendHash, destinationDir, buildPath);

            foreach (var build in versions.data) Copy(build.file, destinationDir, buildPath);
        ...
        private static void Copy(string filename, string destinationDir, string buildPath)
        {
            var from = Settings.GetBuildPath(filename, buildPath);
            if (File.Exists(from))
            {
        ...
        public static BuildRule GetRule()
        {
            return EditorUtility.FindOrCreateAsset<BuildRule>("Assets/Rule.asset");
        }

        public static AtlasInfos GetAtlasInfos()
        {
            return EditorUtility.FindOrCreateAsset<AtlasInfos>("Assets/AtlasInfos.asset");
        }
    }
}